...
String graphFile = "beatles1024.xml";
float zoomFac = 4;
...
class Particle
{
  ...
  Particle(float x, float y, float r, float g, float b, float radius, int id)
  {
    vx = 0;
    vy = 0;
    fx = fy = wt = 0;
...
void addNode(String label, int parentID)
{
  Particle parent = null;
  float x = 0;
  float y = 0;
  for (int p = 0; p < particles.length; p++) {
    if (particles[p].id == parentID) {
      parent = particles[p];
      break;
    }
  }
  
  // BAD SHIT HAPPENED WE TRIED TO ATTACH TO A PARENT THAT DIDN'T EXIST?
  ...
  particles[i] = new Particle(x,y,random(1),random(1),.5+random(.5), rad, id);
